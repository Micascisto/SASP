...
#Set up a temp directory (if it doesn't exist already)
cur_dir=`pwd`
if [[ ! -d temp ]]; then
    mkdir temp
fi
...
#Set up a unique folder to do everything, so it doesn't create mess in the real main directory
mkdir ${projname_h}_${projname_c}
cd ${projname_h}_${projname_c}
main_working_dir=`pwd`

#Copy over the stereo.default file, so it can be edited right after starting the script
cp /disk/qnap-2/MARS/code/modl/TAG_Code/ASP_ISIS_Code/stefano/stereo.default stereo.default
#Same with polar stereographic projection map file
cp /disk/qnap-2/MARS/code/modl/TAG_Code/ASP_ISIS_Code/stefano/mars_polar_stereo.map mars_polar_stereo.map

#Make a folder to do all of the CTX DEM processing in
mkdir CTX_DEM
cp stereo.default CTX_DEM/stereo.default
cp mars_polar_stereo.map CTX_DEM/mars_polar_stereo.map
#Make a folder to do all of the HiRISE DEM processing in
mkdir HiRISE_DEM
cp stereo.default HiRISE_DEM/stereo.default
cp mars_polar_stereo.map HiRISE_DEM/mars_polar_stereo.map
...
if [ ${img_count_c} -eq 2 ]; then
	...
	#USE FULL RESOLUTION:
	cam2map4stereo.py ${image1_c_full}_cal.cub ${image2_c_full}_cal.cub --map=mars_polar_stereo.map
	...
	if [ ${num_mola_pts} -gt 100 ]; then
		...
		point2dem ${projname_c}_tied-trans_reference.tif -o temp -r mars --stereographic --proj-lon 0 --proj-lat 90 --orthoimage --orthoimage-hole-fill-len 10 ${projname_c}-L.tif --nodata-value 0.0
		point2dem ${projname_c}_tied-trans_reference.tif -o temp -r mars --stereographic --proj-lon 0 --proj-lat 90 --dem-hole-fill-len 10 --nodata -32767 --dem-spacing 18 --max-valid-triangulation-error 10
...
if [ ${CTX_DEM_flag} -eq 1 ]; then
	...
	if [ ${img_count_1_h} -eq ${img_download_count_1_h} ] && [ ${img_count_2_h} -eq ${img_download_count_2_h} ]; then
		...
		if [ ${input_res_check_h} -eq 1 ]; then
			#USE FULL RESOLUTION:
			cam2map4stereo.py ${image1_h}_RED.mos_hijitreged.norm.cub ${image2_h}_RED.mos_hijitreged.norm.cub --map=mars_polar_stereo.map
		else
			#DOWNSAMPLE TO INPUT RESOLUTION
			cam2map4stereo.py ${image1_h}_RED.mos_hijitreged.norm.cub ${image2_h}_RED.mos_hijitreged.norm.cub -p MPP -r ${input_res_h} --map=mars_polar_stereo.map
		fi
...
if [ ${HiRISE_DEM_flag} -eq 1 ]; then
	...
	point2dem ${projname_h}_tied-trans_source.tif -o temp -r mars --stereographic --proj-lon 0 --proj-lat 90 --orthoimage --orthoimage-hole-fill-len 10 ${projname_h}-L.tif --nodata-value 0.0
	point2dem ${projname_h}_tied-trans_source.tif -o temp -r mars --stereographic --proj-lon 0 --proj-lat 90 --dem-hole-fill-len 10 --nodata -32767 --dem-spacing ${output_res_h} --max-valid-triangulation-error 5
...
if [ ${HiRISE_DEM_flag} -eq 1 ] && [ ${CTX_DEM_flag} -eq 1 ]; then
	...
	#Now transfer stuff to the main directory
        mv Final_Tied_DEMs/ ${cur_dir}/Final_Tied_DEMs/
        cd ${cur_dir}
	...
	rm -rvf HiRISE_DEM_Tie/
        rm -rvf temp/${projname_h}_${projname_c}
