...
cumindex_loc_h=`echo echo /disk/qnap-2/MARS/orig/supl/HiRISE/HiRISE_cumindex/EDRCUMINDEX.TAB`
main_path_h=`echo http://hirise-pds.lpl.arizona.edu/PDS`
cumindex_loc_c=`echo /disk/staff/tgoudge/Stored_Data/CTX_cumindex/cumindex.tab`
...
if [ ${img_count_c} -eq 2 ]; then
	...
	cp /disk/qnap-2/MARS/code/modl/TAG_code/stereo_dot_default_files/stereo_dot_default_file_CTX_v2.2.0.txt stereo.default
...
if [ ${CTX_DEM_flag} -eq 1 ]; then
	...
	if [ ${img_count_1_h} -eq ${img_download_count_1_h} ] && [ ${img_count_2_h} -eq ${img_download_count_2_h} ]; then
		...
		#NOTE: THIS STEREO.DEFAULT FILE HAS THE DEFAULT PARAMETERS SET BY TAG, AND MAY NEED TO BE CHANGED
		#TO CHANGE, CREATE YOUR OWN STEREO.DEFAULT FILE IN THE WORKING DIRECTORY AND COMMENT OUT THE LINE BELOW
		cp /disk/qnap-2/MARS/code/modl/TAG_code/stereo_dot_default_files/stereo_dot_default_file_v2.2.0.txt stereo.default
