...
# Modified by Stefano Nerozzi (stefano.nerozzi@utexas.edu) [solved issue with stereo.default file]
#
...
if [ ${img_count} -eq 2 ]; then
	...
	if [ ${num_mola_pts} -gt 100 ]; then
		...
		point2dem ${projname}_tied-trans_reference.tif -o temp -r mars --sinusoidal --proj-lon ${clon} --orthoimage ${projname}-L.tif --nodata-value 0.0
		point2dem ${projname}_tied-trans_reference.tif -o temp -r mars --sinusoidal --proj-lon ${clon} --nodata -32767 --dem-spacing ${output_res}
